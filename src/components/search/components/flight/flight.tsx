...
import { setFlightList } from "@/redux/features/flightSlice";
...
const fetchFlightList = async (query: {
  ...
  const response = await axios.get(`http://localhost:4000/flights`, {
    params: {
      departureCity: query?.departureCity?.toLowerCase(),
      arrivalCity: query?.arrivalCity?.toLowerCase(),
      _sort: query?.orderBy,
      _order: query?.order,
    },
  });
...
export const Flight = () => {
  ...
  const {
    ...
    queryKey: ["flightListData", params],
    queryFn: () => fetchFlightList(params),
    enabled: enabled,
  ...
  const onSubmitHandler = handleSubmit((data) => {
    setParams({
      ...params,
      departureCity: getValues("departureCity"),
      arrivalCity: getValues("arrivalCity"),
    });
    setEnabled(true);
